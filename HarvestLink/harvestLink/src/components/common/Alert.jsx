...
const Alert = ({ type = "error", message, onClose }) => {
  ...
  return (
    <div
      className={`${baseClasses} ${typeClasses[type]} flex items-center`}
      role="alert"
    >
      <div
        className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 mr-3 ${iconClasses[type]}`}
      >
        {icons[type]}
      </div>
      ...
      {onClose && (
        <button
          type="button"
          className="ml-auto -mx-1.5 -my-1.5 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 inline-flex h-8 w-8 hover:bg-gray-100 dark:hover:bg-gray-700"
          onClick={onClose}
          aria-label="Close"
        >
